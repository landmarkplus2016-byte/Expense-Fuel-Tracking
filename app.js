...
async function buildSheet(wb, logoId, sheetName, sheetTitle, headers, rows, amtTotal, fuelTotal, kartaTotal, colWidths, empName, account, period) {
  ...
  for (var c = 4; c <= n; c++) {
    ...
    cl.font      = {color:{argb:WHITE},bold:true,size:16,name:'Calibri'};
...
async function exportBalanceSheet() {
  try {
    ...
    // --- Banner ---
    try {
      var bannerEl = document.querySelector('img[alt="Balance Sheet Banner"]');
      if (bannerEl && bannerEl.src.indexOf('base64,') >= 0) {
        doc.addImage(bannerEl.src, 'JPEG', lm, y, cw, 36);
        y += 40;
      }
    } catch(e2) {}
